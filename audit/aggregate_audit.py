...
def main():
    ...
    print(f"{'constraint':22s} " + "  ".join(f"A{i+1}_pass" for i in range(len(sheets)))
          + "   pooled   raw_agree   kappa   n")

    overall_pairs = []
    for k in KEYS:
        per_author_pass = [[] for _ in sheets]
        pairs = []          # (a,b) for kappa/raw-agreement when 2 authors
        ...
        print(f"{k:22s} " + "  ".join(rates) + f"   {pooled_rate}   {rstr}   {kstr}  {n:3d}")
        overall_pairs += pairs

    if len(sheets) == 2 and overall_pairs:
        raw = sum(1 for a, b in overall_pairs if a == b) / len(overall_pairs)
        print(f"\noverall raw agreement (all constraints pooled): {raw*100:.1f}%")
        print(f"overall Cohen's kappa (all constraints pooled): {cohen_kappa(overall_pairs):.3f}")
        print("note: where a constraint is passed by almost every example, kappa is deflated by the "
              "base-rate effect; read pass rate and raw agreement as primary.")
